...
if(detalle.picture === undefined){
  ...
  document.title = `${detalle.category}/${detalle.name}`;
  ...
  return (
    <><div><Container className="d-flex text-center justify-content-center p-5 slide-in-fwd-center"><Breadcrumb listProps={{ className: "justify-content-center" }} className="justify-content-center">
      <Breadcrumb.Item linkAs={Link} linkProps={{ to: "/" }}>Mi tiendita</Breadcrumb.Item>
        ...
        <div className="row">
          <div className="col-sm-6 col-md-6 col-lg-6 push-bit align-middle">
            <span className="position-absolute top-10 end-90 translate-middle badge rounded-pill bg-info">{isInCart && estaEnCarrito(detalle.id) ?
              <><div className="cart-icon2"><FontAwesomeIcon icon={faCartShopping} size="3x" color="white" /><div className="mostrar-cantidadItem">{getItemQuantity(detalle.id)}</div></div></>
              :
              <><div className="cart-icon2"><FontAwesomeIcon icon={faCartShopping} size="3x" color="white" /><div className="mostrar-cantidadItem">0</div></div></>}</span>
